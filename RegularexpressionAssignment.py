...
Actual data: http://py4e-data.dr-chuck.net/regex_sum_345254.txt (There are
 99 values and the sum ends with 459)
...
The file contains much of the text from the introduction of the textbook
 except that random numbers are inserted throughout the text. Here is
 a sample of the output you might see:
...
integers using the re.findall(), looking for a regular expression
 of '[0-9]+' and then converting the extracted strings to integers and
...
import re
hand = open('regex_sum_345254.txt')
numlist = list()
for line in hand:
